...
namespace HungarianMethod
{
    class HungarianMethod
    {
        ...
        public HungarianMethod(int[,] priceMatrix, string taskType)
        {

            //Перевод для удобства работы
            string[,] workingMatrix = IntArrayToStringArray(priceMatrix);
            Result = new string[workingMatrix.GetLength(0), workingMatrix.GetLength(0)];
            ...
            while (zeroStarCount < workingMatrix.GetLength(0))
            {
            ...
            Array.Copy( workingMatrix, 0,this.Result, 0, workingMatrix.Length);
        ...
        public int DisplayResultToDGV(DataGridView dgv, int[,] priceMatrix)
        {
            int FXOpt = 0;
            for (int i = 0; i < this.Result.GetLength(0); i++)
            {
                for (int j = 0; j < this.Result.GetLength(0); j++)
                {
                    if (this.Result[j, i] == "0*")
                    {
